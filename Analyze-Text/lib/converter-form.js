  ...
  if (typeof ConverterForm.convert !== 'function') {
    ConverterForm.convert = function (inID, outID, errID) {
      ...
      // We want TextToIPA to exist, or we can't lookup words
        ...
        var englishTextArray = document.getElementById(inID).value.split(/[ \n()/.?!,;—]/g).filter(Boolean);
        ...
        for (var i in englishTextArray) {
          ...
          var IPAWord = TextToIPA.lookup(englishTextArray[i].toLowerCase().replace(/\'(.*)|\’(.*)/, '').replace(/[^\w\sè]|_/g, '').replace(/\s+/g, ' '));
